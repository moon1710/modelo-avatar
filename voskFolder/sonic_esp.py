...
import pyaudio
import json
from vosk_recognition import Model as VoskModel, KaldiRecognizer
...
def responder_con_voz(respuesta):
    ...
    print(f"Bot (Sonic): {respuesta}")
    engine.say(respuesta)
    engine.runAndWait()

# Contexto del chatbot de Sonic en español
contexto = (
    "Eres Sonic, el erizo azul, famoso por tu velocidad y actitud relajada. "
    "Responde con un tono dinámico, lleno de energía y optimismo, siempre listo para la acción. "
    "Mantén tus respuestas cortas, directas y llenas de entusiasmo. "
    "Demuestra una personalidad amigable y confiada, y nunca pierdas tu espíritu aventurero. "
    "Responde en español."
)
...
def main():
    print("¡Chatbot activado como Sonic! Di 'salir' para terminar la conversación.")
    while True:
        entrada = escuchar_usuario()
        if entrada:
            if entrada.lower() == "salir":
                print("Terminando la conversación. ¡Hasta luego!")
                responder_con_voz("¡Hasta luego! ¡Sigue corriendo a toda velocidad!")
                break
