...
def extract_phonemes(text):
    """Usa eSpeak-NG para obtener fonemas de un texto"""
    try:
        result = subprocess.run(
            ["espeak-ng", "-q", "--ipa", text], capture_output=True, text=True
        )
        phonemes = result.stdout.strip()
        print(f"📢 Fonemas extraídos: {phonemes}")  # 👀 Verifica qué fonemas genera
        return phonemes
...
def lipsync_animation(phonemes, duration=2.5):
    ...
    for phoneme in phoneme_list:
        ...
        print(f"🟢 Fonema detectado: {phoneme} -> Cambiando a visema: {viseme}")  # 👀 Debug
        update_frame(viseme)
        time.sleep(total_time)

    update_frame("neutral")  # Regresa a posición neutral

# ---------------------------
# Función para sintetizar voz y hacer lipsync
# ---------------------------
def speak_response(response):
    """Reproduce la respuesta en audio y hace lipsync"""
    def run_tts():
        ...
        # 🔹 **Reproducir la animación de lipsync**
        lipsync_thread = threading.Thread(target=lipsync_animation, args=(phonemes,))
